...
Run from this directory: `pytest test6A.py -v`
"""
...
_PLOT_PATH = _OUTPUT_DIR / "test6A_error_plot.png"
...
def _report_recorded_errors(request):
    ...
    ax.set_title("Test 6A Error Histogram")
    ...
    terminal.write_line("")
    terminal.write_sep("=", "test6A kick-error report")
    terminal.write_line(
...
@pytest.mark.verification_suite("6A")
@pytest.mark.parametrize(
    "trial_number",
    _TRIAL_NUMBERS,
    ids=[f"trial_{n}" for n in _TRIAL_NUMBERS],
)
def test6A(run_c, trial_number: int):
    """Test 6A: kicking accuracy."""
